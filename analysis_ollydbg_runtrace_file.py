...
def build_control_flow_graph(blocks, is_order, origin_instruction_order):
    ...
    if is_order:
        ...
        # 说明两个代码块存在连接关系
        for index, block in enumerate(blocks):
            last_instruction_addr = block.instructions[-1]['address']
            for other_index, other_block in enumerate(blocks):
                if index != other_index:
                    first_instruction_addr = other_block.instructions[0]['address']
                    # 开始在原始指令流里面匹配
                    for i, addr in enumerate(origin_instruction_order):
                        if i < len(origin_instruction_order)-1 and addr == last_instruction_addr and origin_instruction_order[i+1] == first_instruction_addr:
                            # 成功匹配，构建两个块的连接关系
...
def merge_serial_blocks(blocks, block_counts):
    ...
    for i in range(len(blocks)):
        ...
        while can_merge:
            ...
            # 对于VMP3.x版本，如果代码块尾部是jmp esi或者push esi + ret，则不要合并
            if blocks[current_idx].instructions[-1]["command"] == "jmp esi":
                can_merge = False
                break
            if len(blocks[current_idx].instructions) >= 2 and blocks[current_idx].instructions[-2]["command"] == "push esi" and blocks[current_idx].instructions[-1]["command"] in ["ret", "retn"]:
                can_merge = False
                break
...
def generate_dot_file(blocks, block_counts, output_file):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for idx, block in enumerate(blocks):
            ...
            # 根据执行次数决定标题样式
            header_bgcolor = "#FF0000" if count > 1 else "#e0e0e0"
            header_fontcolor = "white" if count > 1 else "black"
            
            # 构建HTML表格
            label = ['<<table border="0" cellborder="1" cellspacing="0">']
            label.append(
                f'<tr><td colspan="2" align="center" bgcolor="{header_bgcolor}">'
                f'<font color="{header_fontcolor}"><b>[ID: {idx}] {block.block_name} (Executed: {count} time{"s" if count > 1 else ""})</b></font>'
                f'</td></tr>'
            )
    ...
    with open(path, "w", encoding="utf-8") as f:
        for i, b in enumerate(blocks):
            ...
            for inst in b.instructions:
                f.write(f"{inst['address'].zfill(8)} {inst['command']}\n")
            f.write("\n")
...
def reorder_merged_blocks(merged_blocks):
    ...
    # 将代码块出现是顺序记录到文件中
    with open(target_dir + "/block_order.txt", "w", encoding="utf-8") as f:
...
if __name__ == '__main__':
    ...
    target_file = "E:\\rtrace_3.5.0.txt"
    target_dir = target_file + "_result"
    if not os.path.exists(target_dir):
        os.mkdir(target_dir)

    entries = parse_trace_file(target_file)
    ...
    save_blocks_txt(blocks, target_dir + "/rtrace_segments_full.txt")
    
    # 去重处理
    unique_blocks, origin_index_to_unique_index, block_counts = deduplicate_blocks(blocks)
    print(f"去重后代码块数量: {len(unique_blocks)}")
    save_blocks_txt(unique_blocks, target_dir + "/rtrace_segments_dedup.txt")
    ...
    save_blocks_txt(merged_blocks, target_dir + "/rtrace_segments_merged.txt")
